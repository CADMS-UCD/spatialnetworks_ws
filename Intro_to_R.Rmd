...
## Scatterplot.  
This is one of the most popular kind of plots, it is useful to represent relationship between two continous variables.  
We can achieve the same result in different ways. The first one is better when we have our variables in the same dataset, and the second one when we have data from diferent objects. (the length of the two vectors has to be the same).  
...
To select a variables from a data.frame we use `$`. For exmaple if we want to see the variable farm type from our PRRS dataset, we use `PRRS$farm_type`.  
...
## Boxplots  
Boxplots are great to show the distribution of a continous variable. we can use it to show only one variable, or multiple variables.  
```{r}
# Only one variable
boxplot(x = PRRS$Age)
```
...
# Fisrt we will get a table with the results stratified by sex
...
**Excercise:** Create a bar plot of the results stratified by the farm type:  
...
To create a time series we will need to reformat the data a little bit so R can do wht we want.  
```{r}
