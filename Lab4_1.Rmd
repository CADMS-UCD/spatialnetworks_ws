...
*Trying to convert everything to tidygraph*
...
Let's start by importing the data for edges, node attributes, and the shape file of the area of study.
...
```{r tidygraph network}
lapply(unique(net$w_onset), function(x){
  net %>% 
    as_tbl_graph() %E>% 
    filter(w_onset == x) %N>% 
  ...
  data.frame() %>% 
  `colnames<-`('Density') %>% 
  mutate(week = unique(net$w_onset)) %>% 
  ggplot() +
  geom_line(aes(x = week, y = Density), col = 'grey20', lwd = 1) +
  theme_minimal()
```
...
lapply(unique(net$w_onset), function(x){
  ...
  unlist() %>% 
  data.frame() %>% 
  `colnames<-`('Reciprocity') %>% 
  mutate(week = unique(net$w_onset)) %>% 
  ggplot() +
  geom_line(aes(x = week, y = Reciprocity), col = 'grey10', lwd = 1) +
  theme_minimal()
```
