...
```{r include=FALSE}
knitr::opts_chunk$set(warninr = F, message = F)
```

# 1 Deteccion de comunidades

```{r}
# Si empezamos una nueva sesion tendremosque cargar las librerias y los datos de nuevo
...
Vamos a utilizar el algoritmo walktrap para detectar comunidades en la red observada usando la funcion `walktrap()`.   

```{r}
# Primero simplificamos la red
c<-simplify(net)
# Despues usamos 
wc <- cluster_walktrap(c)
# Identificar el numero de comunidad al que pertenece:
membership(wc)
# Graficar la red y las comunidades
plot(wc, c, edge.arrow.size=.2,vertex.size=4,vertex.label=NA)
```


## 2 Representacion espacial de la red


```{r}
# cargar los paquetes
library(sf) # para objetos espaciales
...
Transformamos los nodos a un objeto espacial:  

```{r Transform nodes to spatial}
NodeSp <- node %>% # Aqui va el data.frame
  st_as_sf(coords = c("long", "lat"), # Nombre de las variables que indican las coordenadas
           crs = st_crs(iowa)) # El sistema de coordenadas que usamos
```
...
```{r}
map <- ggplot() +
  geom_sf(data = iowa, color="gray",fill="white", size=0.2)
```


## 2.2 Graficar los nodos

```{r}
#plot nodes#
# Agregamos los puntos solamente
map+geom_sf(data = NodeSp, color = "red")

# Agregamos los puntos con el tipo de granja
map+
  geom_sf(data = NodeSp, aes(color = farm_type))

# ajustamos el tamaño
map2 <- map+
  geom_sf(data = NodeSp, aes(color = farm_type), size = 3)
...
```{r}
# la funcion geom?segment agrega una linea recta entre dos coordenadasÑ
map +
