...
```{r temporal net density}
x <- sort(unique(net$week))
...
sapply(x, function(x){
  ...
  data.frame(week = x, density = .) %>% 
  ggplot() +
  geom_path(aes(x = week, y = density)) +
  labs(title = 'Weekly network density')
...
sapply(x, function(x){
  ...
  data.frame(week = x, reciprocity = .) %>% 
  ggplot() +
  geom_path(aes(x = week, y = reciprocity)) +
  labs(title = 'Weekly network reciprocity')
```
